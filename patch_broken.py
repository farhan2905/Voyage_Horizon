...
broke_files = ['australia.html', 'ireland.html', 'netherlands.html', 'new-zealand.html']

country_names = {
    'australia.html': 'Australia',
    'ireland.html': 'Ireland',
    'netherlands.html': 'Netherlands',
    'new-zealand.html': 'New Zealand'
}
...
# ALSO we need the fee and PSW from study_visa_data
study_visa_data = {
    'australia.html': {
...
for fname in broke_files:
    fp = os.path.join(base, fname)
    with open(fp, 'r', encoding='utf-8') as f:
        html = f.read()
    
    # Trim off everything after the last </section> to ensure we are appending cleanly
    idx = html.rfind('</section>')
    if idx != -1:
        html = html[:idx + 10]
    else:
        # just cut before </body>
        idx = html.rfind('</body>')
        if idx != -1:
            html = html[:idx]
    
    country = country_names[fname]
    d = study_visa_data[fname]
    
    new_html = html + make_psw_section_sv(d) + make_fee_section_sv(d) + make_roadmap_html(country) + make_cta_html(country) + make_form_html(country) + FOOTER_HTML
    
    with open(fp, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print(f"Patched {fname}")

print("Done patching broken pages.")
